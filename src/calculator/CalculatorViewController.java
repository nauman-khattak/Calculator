...
public class CalculatorViewController extends JPanel {
    ...
    ButtonGroup buttonGroup; //buttonGroup for checkbox and readiobuttons
    ...
    //if result is displayed on display2 then this field is set to true
    //When this is true then text is placed on display2. When this is false then text is concatenated to already
    //placed text on the display2
    boolean resultDisplayed = false;

    //checks whether operator is present on display1 or not.
    //this prevents multiple operators to be added one after another on display1
    boolean operatorAdded = false;

    //when set to true, backspace wont work
    //when set to false, backspace would work
    boolean backspaceDisabled = false;

    //a helper to check if error occured or not. If error occured then keypad buttons and equal button will not response to click events.
    boolean errorCheck = false;
    
    //this will store display1 text when result is displayed on display2
    String display1Text = "";
    
    //a helper to check if its time to save display1 text in a String display1Text
    ...
    private class Controller implements ActionListener {
        ...
        //whenever a button, radioButton, checkBox is clicked this method is invoked.
        @Override
        public void actionPerformed(ActionEvent e) {
            ...
            switch (actionCommand) {
                ...
                case "dot":
                    ...
                    //if error message is displayed on display2 then [0-9] buttons and dot button wont response to click events
                    if (errorCheck) {
                        return;
                    }
                    
                    //setting values on dsiplay2
                    ...
                    } else if (resultDisplayed) {//if previous result is visible on display2 then clear it and set resultDisplayed tp false
                        display2.setText(keypadText[Arrays.asList(actionCommandText).indexOf(actionCommand)]);
                        display1.setText("");
                        resultDisplayed = false;
                    } else {
                        //if numbers are already present on display2 then concat pressed number/decimalPoint to it
                ...
                case "add":
                case "minus":
                    //if error message is displayed on display2 then arithmetic operator buttons wont response to click events
                    if (errorCheck) {
                        return;
                    }
                    
                    //no arithmetic operator can be added alone to display2 except negative arithmetic operator i.e "-"
                    ...
                    //except minus, no arithmetic operator can be added alone to display2
                    ...
                    break;
                case "negate":
                    //if error message is displayed on display2 then negate button wont response to click events
                    if (errorCheck) {
                        return;
                    }
